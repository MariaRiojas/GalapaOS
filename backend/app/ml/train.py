...
from ..config import BATCH_SIZE, LEARNING_RATE, PATIENCE, MAX_EPOCHS, CHECKPOINTS_DIR
...
def train_model(model, train_ds, val_ds, target_name, rnn_type="lstm", device="cpu"):
    ...
    print(f"  Training {rnn_type.upper()} for {target_name}")
    print(f"    Train: {len(train_ds)} samples, pos_rate: {pos_count/len(train_ds):.4f}")
    ...
    ckpt_path = CHECKPOINTS_DIR / f"{rnn_type}_{target_name}.pt"
